...
FAILURE_TIME = list(range(0, 25))  # 故障発生時間（0-23時）
...
def create_failure_scenario_parallel_safe(cs_config_dict, trial_params, trial_number, outer_parallel_count, combination_id):
    ...
    try:
        ...
        with Pool(processes=max_processes, initializer=init_worker_globals, initargs=(SAVE_DIR,)) as pool:
...
if __name__ == "__main__":
    ...
    SAVE_DIR = current_time + "_FAILURE"  # 故障シナリオ用のディレクトリ名
    # SAVE_DIR = "20250827_1335_FAILURE"  # ここは適宜変更してください
    os.makedirs(SAVE_DIR, exist_ok=True)
    
    # 並列処理設定
    BATCH_SIZE = 8          # バッチサイズ（同時に処理するトライアル数）
    OUTER_PARALLEL = 4      # outer_parallel数（トライアルごとの故障シナリオ並列度）
    TOTAL_TRIALS = 1000      # 総トライアル数
    TIMEOUT = 60 * 60 * 12  # 12時間タイムアウト
